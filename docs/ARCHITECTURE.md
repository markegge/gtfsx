...
| `GET /api/me/twofa`, `POST /api/me/twofa/enable` · `/disable` · `/confirm`, `POST /api/me/phone[/verify]` (stubs → `sms_unavailable`) | Per-user 2FA settings (off by default; enable/disable each round-trip a code). Org-side: `PATCH /api/orgs/:id` accepts `require_2fa` (admin+) — members of a requiring org get an email challenge at login even if not enrolled, and can't disable 2FA |
