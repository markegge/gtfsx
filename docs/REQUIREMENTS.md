...
- 🔲 SMS 2FA via **Twilio Verify** (worker endpoints + UI stubbed behind `sms_unavailable`; needs Twilio account setup — paid account + Trust Hub profile + Verify Service, no A2P 10DLC/carrier application required for verification-only traffic — then `TWILIO_ACCOUNT_SID`/`TWILIO_AUTH_TOKEN`/`TWILIO_VERIFY_SERVICE_SID` secrets).
