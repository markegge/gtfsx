...
- ✅ **Opt-in `ext_ntd_id` column on `agency.txt`** at export — carries the project's NTD ID inside the zip. **Provisional extension field, NOT part of the GTFS spec** (a proposal for a standard NTD ID field is pending; column name may change — tracked as [#62](https://github.com/GTFS-X/gtfsx/issues/62)). Default **OFF**; when off, `agency.txt` is byte-identical to a no-NTD export. Written **only for single-agency feeds** — the NTD ID is per-project and per-agency NTD IDs aren't modeled, so stamping one id onto every row of a multi-agency `agency.txt` would assert something false; the opt-in no-ops and the Export dialog explains why (`ntdIdExportStatus()` in `gtfsExport.ts` is the single source of truth, shared by the exporter and the dialog). Read back on import (and the opt-in re-enables itself), so the column round-trips.
...
- ✅ Sidecar `feeds.*/<slug>/feed_info.json` with title, description, effective dates, version id, contact, distribution targets, registered RT feeds — plus `ntd_id` and `license_spdx_identifier` when the publisher set them (both keys omitted entirely when unset, so a consumer sees "absent" rather than an explicit null).
- ✅ **Feed identity at publish** — the Publish panel carries two optional fields projected onto the publication: the agency's **NTD ID** (§4.3) and the feed's **license** (SPDX short id; `CC0-1.0` / `CC-BY-4.0` / `ODbL-1.0` / unset — we never guess one). The NTD ID lives in editor feed state (`projectSlice.ntdId`, the source of truth, synced to IndexedDB + the R2 state snapshot); publish additionally projects it onto `feed_project.ntd_id` (migration 0024) so the public feeds origin can serve it without inflating the state blob. **NTD IDs are 1–5 digit strings with significant leading zeros** — carried as text end to end and never `Number()`-coerced.
...
- ✅ **NTD ID + the FTA P-50 crosswalk.** FTA proposed forcing `agency_id` == NTD ID, withdrew it (July 2025), and now crosswalks feeds → NTD IDs itself via the enhanced **P-50** (Transit Agency Identification) form, which collects the feed's stable URL plus the `agency_id`/`agency_name` pairs inside it. GTFS·X carries the NTD ID through publication (§4.1) so the crosswalk survives, and surfaces an **NTD / P-50 helper panel** (`NtdP50Panel.tsx`, shown once a feed is published): the NTD ID, the canonical feed URL, and every `agency_id` + `agency_name` in the feed, each copy-ready — exactly the fields the form asks for. Read entirely from existing state; no new endpoint. Multi-agency feeds get an explicit caveat: the NTD ID identifies the *reporting* agency for the feed as a whole, and a per-agency NTD ID is not modeled.
- ✅ **DMFR document per published feed** — `GET feeds.*/<slug>/dmfr.json`, a [Distributed Mobility Feed Registry](https://dmfr.transit.land/) v0.5.1 document (schema vendored + asserted against in `worker/__tests__/`). Lets a publisher hand **one URL** to Transitland / the Mobility Database and land in their pipelines with the NTD crosswalk already attached, on `operators[].tags.us_ntd_id` (the tag transitland-atlas uses). Also carries the feed's SPDX license and a companion `gtfs-rt` entry (one `urls` field per registered RT message type; omitted when the project has no RT feeds). Judgment calls, since the schema is stricter than our data model: the required `operator.onestop_id` is **derived** following Transitland's documented `o-<geohash>-<name>` convention from the feed's stop centroid (rather than emitting a placeholder), and when the feed has no usable stop coordinates the optional `operators` container is omitted entirely and the NTD ID falls back to the feed's free-form `tags` — the crosswalk survives either way. `operator.associated_feeds[].gtfs_agency_id` is deliberately **not** emitted (we don't model which agency in a multi-agency feed is the NTD reporter, and asserting an unbacked mapping would be worse than leaving it to the registry maintainer).
