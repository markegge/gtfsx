...
/** Names already generated by promotion — used to detect nesting. */
const PRIOR_BASELINE_RE = /^Baseline \(before .*\)$/;
...
/** Make `name` unique among `taken`, appending " (2)", " (3)", … if needed. */
...
/**
 * Name for the old baseline once a variant is promoted over it. Normally
 * "Baseline (before {promoted})", but if the promoted variant is ITSELF a
 * "Baseline (before …)" (a former baseline), that plain scheme would nest
 * ("Baseline (before Baseline (before …))") — so fall back to a timestamped
 * "Baseline (before promotion — {when})". De-duplicated against `existingNames`
 * (the names that will remain in the list) with a " (n)" counter. Pure.
 */
export function priorBaselineName(
  promotedName: string,
  existingNames: string[] = [],
  now: number = nowMs(),
): string {
  const base = PRIOR_BASELINE_RE.test(promotedName.trim())
    ? `Baseline (before promotion — ${stampShort(now)})`
    : `Baseline (before ${promotedName})`;
...
export function promoteToBaseline(variantId: string): void {
  ...
  // Name the preserved old baseline, avoiding nesting and collisions with the
  // names that will remain: the new "Baseline" + every kept non-baseline variant
  // (the promoted one is dropped; the old baseline is the one being renamed).
  const remainingNames = ['Baseline', ...useStore.getState().variants
    .filter((v) => v.id !== variantId && !v.baseline)
    .map((v) => v.name)];
  const priorName = priorBaselineName(promoted.name, remainingNames, t);
  ...
  for (const v of useStore.getState().variants) {
    ...
    if (v.baseline) {
      ...
      rebuilt.push({ ...v, baseline: false, name: priorName, modifiedAt: t });
    } else {
