...
async function createVersionWithFeedInfo(client: TestClient, projectId: string): Promise<{ version: { id: string } }> {
...
async function publishMultipart(
  ...
  const form = new FormData();
  form.append('meta', JSON.stringify({ versionId }));
...
describe('feeds.* distribution handler', () => {
  ...
  it('GET feeds.*/<slug>/gtfs.zip serves the published ZIP with cache headers', async () => {
    ...
    const v = await createVersionWithFeedInfo(client, proj.id);
    const zipBytes = new TextEncoder().encode('PKZIPzipzipzip');
    await publishMultipart(client, proj.id, v.version.id, zipBytes);
  ...
  it('GET feeds.*/<slug>/feed_info.json returns the sidecar with RT feeds + distribution', async () => {
    ...
    await publishMultipart(client, proj.id, v.version.id, new Uint8Array([1, 2, 3]));
    ...
    expect(body.version_id).toBe(v.version.id);
